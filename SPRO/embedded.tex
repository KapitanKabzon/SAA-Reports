...
\subsection*{Frame}
In the design, the choice of the frame unquestionably was an important choice upon which the features and the rest of the design would be based on. In order to prepare for this choice, research of automobile projects was done. The results indicated that the most common feature was that the RC cars, which were modified and adjust to the desired options, or vehicle structure-based on aluminum segments were designed to be adjusted to one's needs. Firstly, builidng upon a RC platform was considered, so that an existing working drive mechanism and include food serving product, nevertheless, no vehicle of desired size could be found. Therefore, platform out of aluminum segments was chosen. Aluminum profiles, which are easily accessible allow to build a stronger frame, keeping the weight of the robot down. To choose the right series of a profile equation supplied by the manufacturer was used to check the strength and deflection of the profile under load. The calculation showed that profile 20$\multiply$20 with 380mm span deflects 1.1521mm at 2000N load\footnote{(TODOCalculation in appendix-USB stick)}. Which is a very good results and acceptable for further development.

 
The choice of frame was Aluminum Profile series 5 20$\multiply$20 which is ideal for the application. It delivers rigidity and allows for very easy re-development and re-construction of the model. In the concept design phase, it was established that robot should be easy to mount. Therefore, it was decided to build the frame with foor that can be easily disassembled to make necessary changes if needed. We came up with very simple structure: a rectangular box with a square base 420mm x 420mm x 300 mm and 㦬oor䠩n the middle. The Construction is assembled from 12 x 380mm and 4 x 300 mm Item aluminum profile series 5 20x20.  The span between floor and base line is 60 mm. It was thought to be a 㢲ain level䠷hich would contain all the electronics, including PCB, H bridges, Arduinos, Raspberry Pi. The span between the drawer level and the top is 180mm which is enough space to fit in the food container and its drive. In order to screw the frame. We had to modify the profiles. The 380mm profile we tapped from both sides, making a 30 mm thread M5. In the 300mm Profile we set the position of the ゲain䬔 Drawer䬔 Top䠬evels. We had to drill the 6 mm bore through which we could adjust the fastening set.


Fastening set is screwed and metal plate which secures the profile in the set position. We have used 24 of them to build the frame. This set could be ordered from workshop and it transpired to be a surprisingly expensive part. The university workshop price was 300 DKK for 30 pieces. To see the Frame assembly in detail please see the presentation NAME video located on the USB stick *location* 

\subsection*{Floor Levels}
The slots in the aluminum profile allow to fit in the floor- a wooden or aluminum panel on which is mounted drives and electronics, from the inside without any mounting. In our design we had to use the 6 mm laser cut MDF which is widely available in the student workshop. The slots in aluminum profile of our choice are 5 mm width and 6 mm deep, which caused us a problem of fitting them together. (In the original design, we asked for the Bosch aluminum profile 20x20 which has a 6 mm slot, perfect for our need and the MDF available in the workshop, nevertheless we got the Item profiles which have 5 mm slot.) This misunderstanding imposed changes in the CAD and adjusting the MDF by grinding the edges 1 mm or just press-fit the wood which is very elastic. The grinding had to be done with extreme cautiousness since the tools and machines in the workshop are rather raddled.


Firstly, the brain-level-floor was designed entirely from MDF on which we would place the drive and electronics. After first try of testing and aligning the drive, we have noticed that the floor can easily be pressed and bent because of the low modulus of elasticity and big span between supports. This phenomenon, causes the wheel to tilt under load, over 10ࠤegrees when measured in the workshop environment. The redesign must have been done in order to prevent the unstable drive. We carefully redesigned and tested another floor layout, upgrading each time. The best solution was to create a 100 mm Plate with pre-cut holes for easier component aligning. The, narrowed, mounting plate was rigid and prevented any unwanted wobble, bending or vibration of the drivetrain.


The MDF used were sufficient for our need, nevertheless, we decided to water jet cut 5 mm aluminum plate to protect ourselves even more against misalignments in the drive. This treatment increased the weight of the plate, however, stiffness, protection and more appealing look outweigh our decision.


The second part of the brain level floor was expected to have freewheel and hold the electronic components. At the beginning, it was only designed to have a bore for the wires and it was expected to be redesigned at the stage where all the electronics and its placement is known. The additional unforeseen change was integrating a fan into the design. Next step, to improve our design is to place to additional freewheel on the back so more stability is achieved when the drawer is in its furthest deviation.


On the Drawer floor level, we used the simple two plates placed parallel to the direction of the drawer slides. As a consequence, to use MDF to create a drawer, we expected that the dimensions will not be precise, therefore, in order to make holes in the right location for the drawer drive, firstly we had to align the drawer with drawer drive and later on laser-cut the final version. Nevertheless, the laser cutter was not available for the last days of the projects weeks and we could not get the desired, upgraded version of the model at the time. On this floor, there was no need to redesign the original idea. The motor was very close to the edge and we experienced no wobble or unexpected movement. (TODOmovie)
...
The front plate is solely mounted to place the ultrasonic sensors with certain angles, so we can cover a bigger area in front of the automobile. The plate is laser-cut from MDF and have pre-cut holes to plate the L-shaped ultrasonic holders and bore for the wires. The panel is screwed to the profile by means of t-nuts. 


\subsection*{Drawer}
One of the features of our robot was to deliver ordered food to the guest. The best idea we thought off, was to construct some kind of container that is big enough to fit a plate O230mm and is easy to make it mobile. The simplest idea we had, was to design a drawer, a box on which mechanical system can be mounted with no trouble to make it transportable. The biggest problem was to find the rail system which would be smooth and accessible for us to order or manufacture in the workshop. The first idea was to create a roller bearings- a bearing fitted on the grinded/ smooth on side threaded rod that can be simply secured to the box. After creating a sample and testing how it behaves, we were impressed with the smoothness, nevertheless, itⳠnot the solution that we can easily change and redesign in the further part of the development and prototyping process. Moreover, we could not find parts online to order the C-rail in the right size to create a path for it. The additional disadvantage was that it did not offer the required stability, hence we discarded this idea. 


The model 2, was designed to take advantage of already finished product as sliders that allow for the smooth movement of a drawer and stability not to damage the goods. The idea was to create a box with pre-cut holes for the slider and the drive, mechanical system. The final choice was to use rack and pinion system, however, we thought of the systems that asserts more stable and leveled travel. The rack and pinion made our decision, since we can simple laser-cut everything in the workshop. One of the criteria of the model 2, was to make the drawer as big as possible and utilize all the space in the frame. Therefore, it was decided to place the drivetrain on the bottom of the drawer and place the motor in the vertical position as shown in the assembly Motor Mount 944D ver 1 and 2. During the redesign stage, the decision was made on isolating the brain level with electronics and make the drawer very modular, so we can easily stack them together and make the design more future proof.


Fulfilling the criteria, we had to implement changes and the model 3 was born. In the final design, the drawer-box is made from puzzle-like-segments that are glued together (instead of connecting with aluminum L-profile). The side of the container has pre-cut holes that allow to easily mount the slider and slots to adjust the position of the rack, which is placed on the side. The drive is located underneath on the drawer floor. The drawer mounted on the slides can be extended to 265mm which is shorter then datasheet provided by the supplier.


The assembly in detail is shown in the Drawer Assembly Presentation video (TODO?). 

\subsection*{Main Drive}
In our design we used the differential drive, a three-wheel-drive mechanism, two independently driven wheel via motors, which are collinear on the axis and third freewheel to maintain the balance. In our design we went with two 1:100 919DLN motor supplied from the university. These are the strongest motor that is available to us. The drivetrain is constructed from the motor placed on the bottom of the brain floor aluminum plate. Since, our wheel has the 8mm bore and motor 6 mm shaft we had used the coupling with different size. The decision was made to apply elastic couplings, we had in our own stock, 6mm/8mm, which were a perfect fit for our model. The reason behind the elastic coupling was to compensate for small misalignment or vibration going from wheel to motor or vice versa, at least minimize this effect. To secure the 8mm axle we used the pillow block bearing which would take the unnecessary load of the motor gearbox and frame. At first, we wanted to use only one mounted on the profile with the t-nuts, however, with tilting experienced with the first design of the floor, just to prevent any tilt, second one was used right after the coupling. Between two pillow block bearing is placed an encoder which is positioned in the middle of the pre- water-jet-cut-slot. The encoder is screwed to the flange, manufactured in the workshop on the lathe machine, by use of which is locked to the axle. After the second pillow block, there is a wheel screw to the flange, manufactured in the workshop on the lathe machine (the flange was made incorrectly, outer diameter was 10mm to small, so we had to create a template, laser-cut MDF, to place the holes for mounting, since this imprecise technique was used we experienced small misalignment and the screw does not fit properly) TODO PIC . The motor and pillow block are placed on the spacer to make them leveled with respect of the wheel.

\subsection*{Holders}
In our design we use two types of holders. One, which hold the slide and one, which hold the ultrasonic. 	The slide holders are crucial part of our design, because they are load carrying elements, the drawer and whatever guest ordered. We have designed two types of the slide holders, in respect of the asymmetry of the outer slide, different hole sizes and placement (heavily based on the datasheet based on the accuride datasheet and cad model). The first 3d printed, were weak, they have been printed in the wrong position and had 30\% infill which was definitely too small to create any kind acceptable holder. After collaborating with the 3d-print-team, we have implemented changes in 3d model; make is shorter, thicker ribs, more fillets, hence the architecture of the 3d print and set settings to the 100\% infill, and 3d-printed upgraded version. The result was a robust holder.


The ultrasonic sensor holders where design in two different shapes, flat for walls, and L-shaped for the front plate. Both are designed according to the sensor datasheet and measurements taken by caliper. The wall ultrasonic holder is designed to fit the holes and prepared slots in the brain level side and rear walls. The L-shaped ultrasonic holder was constructed to fit on the front plate and hold the sensor with 90-degree angle. In our robot we have used 9 ultrasonic sensors; 6 of the are placed in pairs of two on the side and rear walls to detect wall and unforeseen obstacles and 3 on the front plate to cover the bigger area of the main driving direction.


\subsection*{Covers}
Appealing look was not our main focus in this project, bellhop robot size and shape was mainly defined by the choice of the material use to assemble the frame. Functionality of the design was the key; hence it was decided to divide robot into two parts; brain level with electrical components and food container drawer, to distinguish those two pats we designed different covers/wall to protect and secure before unwanted hands and objects. In the brain level we combined look with functionality and went with plexiglass (5 mm plexiglass sheet available in the workshop fits perfectly in the aluminum profile slots) walls with slots for ultrasonic sensors, the transparent acrylic displays the electronic components inside what is an interesting view to look at. In the drawer floor level, we decided to cover with 3 mm MDF. Two side are plain for simple look (just with holes for mounting) and two, on the drawer travel side, are with pre-cut shapes allowing to drawer to slide out and protect people to pull the drawer out. At first we thought to use locking mechanism to secure the drawer against unwanted, nosey hands, however the idea with shape just big enough for the drawer was good-enough. It fulfilled all the criteria we established. Testing showed that is impossible (very hard) to just open a drawer. Top covers are laser cut from 3 mm MDF. Due to size limitation of the MDF sheet we had to use two panel to cover top area. One side is plain with holes for mounting, the other is design for human interface. It contains holes for mounting and slots for LCD screens and buttons.


\subsection*{Wheel and Sliders}
In the bellhop automobile, two types of wheel were used; for the drive, we have used polypropylene grey wheel, primarily the use of trolleys, nevertheless castor was the perfect fit for us. It can handle 80 kg load and weights only 130g, it does not have strong grip, but on the surfaces we have tested (at university) it is enough. The freewheel is made of an old baggage suitcase. Sadly, we do not have a datasheet for it, however it works perfectly and we the use of caliper we could design and manufacture mounting plates for it. 


\subsection*{Sliders}
In search of the sliders for the drawer we have selected two-way travel Accuride 2026. The reason for it was that we could find datasheet and CAD model and the specification met our requirements; 283.4 mm extension length and 45 kg load and weight of 740g they promised to provide a smooth travel of the drawer.


\subsection*{T-nuts}
T-nuts are made to fit the aluminum profile slot. The threaded bore included in the design provides the connection between built-on part and the aluminum profile via screws.


T-nut primarily are supplied by the manufacturer of the aluminum profiles, however the price of the connecting element turned out to be very expensive. With respect to the very high price we have tried to create a CAD model and 3d print it, nevertheless the quality of the first created parts were not sufficient enough. Necessary adjustments were made with the 3d print set up what resulted, in part that could be used to attach something on the aluminum profile. Although, we obtained the desired effect, the concern appeared, that the material used to 3d print is not strong enough to hold our key elements, especially pillow block bearings.


\subsection*{Movement Principle}
The mechanical drive system we have applied is very simple concept.  Controlling the velocity of the of two motors, their difference, we can drive automobile in any desired path and direction. The built requires two motors, collinear mounted on an axis and additional freewheel. The third wheel keeps the balance. Principles of movement: If the angular velocities are equal, the same rpm and direction on the wheel, robot drives straight or backwards, depending of clockwise or counterclockwise direction. If the velocities are equal, but in the opposite direction, robot spins around its vertical axis, so we turn 㯮 the spot䬠if one wheel is active and second is passive and block the robot will turn around the contact point of the passive wheels, in our case the polypropylene rubber does not have sufficient friction force, grip and tends to create offset to the turn (if the speeds are chosen correctly we can perform a 90 degree turn). If angular velocities are different and motors are spinning in the same direction, then we can turn the automobile left or right, goes curve motion. Combining those motions we can create a full drive and make the robot to follow the path we want. Advantages are that control system is simple and mechanics is robust and sound. The main disadvantage is that nothing is perfect and neither motors or wheels so we can never achieve equal angular velocities. If the robot does not drive as desired, we can make an adjustment and correction factor. We chose to apply the encoder which would control revolutions of the wheel.
...
% References  TODO
% http://www.pehama.dk/download/wmh_aluminiums_profilsystem/#p=79
% http://poltech24.pl/kp08-aluminiowa-miniaturowa-podpora-walka-fi-8mm-p-502.html
% http://poltech24.pl/precyzyjny-walek-liniowy-fi-4-mm-l500mm-prowadnica-cnc-p-149.html
% http://poltech24.pl/sprzeglo-bezluzowe-elastyczne-cnc-d20l25-4x4-mm-p-1405.html
...
% http://www.accuride-europe.com/pdf/2026.pdf
% http://www.robotc.net/wikiarchive/Tutorials/Arduino_Projects/Additional_Info/Turning_Calculations
% http://www.engineersedge.com/gears/basic_rack_tooth_gear_profiles_din_867_13217.htm
...
% CAD file:
% https://grabcad.com/library/castor-wheel-50mm-dia-x-19-mm-wide-1
% http://product.item24.de/en/products/product-catalogue/productdetails/products/construction-profiles-5/profile-5-20x20-natural-37003.html
% https://grabcad.com/library/lockless-on-off-push-button-switch-press-reset-switch-pbs-110-1
% https://grabcad.com/library/hc-sr04-ultrasonic-sensor-1
\newpage
\section{Electronics}
\begin{figure}[h]
  \centering
...
where A is the Arduino Nano microcontroller ID number, as described in TODO:USART DESCRIBE IT and x is the specific error code that pertains to the type of error detected. For example, the list of possible error codes that the LCD Arduino may return are at follows:
\begin{labeling}{usart examples}
...
One thing that can not be completely certain is that what is measured the truly the right RPM, as we there is no tachometer in the SDU workshop that works precisely enough, the one tachometer the workshop has is the one which has flickering light at different frequencies, with which confirmed that the RPM measured by the Arduino is roughly the same ($\approx$140 RPM on the tachometer, and 140 RPM on the Arduino). Another precision point would be the water-jet machine, as the holes for the axle to go through are supposed to be 9mm, but in reality after measuring with calliper they were 8.7mm, which leads to question if the encoders are truly what we designed them to be from edge to edge---24$^\circ$.
...
The technical review was given by a Senior Software Architect, working in a local company.
...
\newpage
\section{Conclusion}
TODO Cat REFERENCE TECHNICAL REVIEW
\newpage
